...
try {
    await child_process.promises.execFile('git', ['clone', 'https://github.com/microsoft/vscode', '.tmp', '--depth', '1'])
    await fs.promises.copyFile(
        path.join('.tmp', 'src', 'vs', 'base', 'browser', 'ui', 'codicons', 'codicon', 'codicon.ttf'), 
...
try {
    // Git clone
    await child_process.promises.execFile('git', ['clone', 'https://github.com/microsoft/vscode-loc', '.tmp', '--depth', '1'])

    // Update package.json
    let from_package = JSON.parse((await fs.promises.readFile(path.join('.tmp', 'i18n', 'vscode-language-pack-zh-hans', 'package.json'))).toString())
    ...
    let main = JSON.parse(
        (await fs.promises.readFile(
            path.join('.tmp', 'i18n', 'vscode-language-pack-zh-hans', 'translations', 'main.i18n.json')
        )).toString()
    )
    ...
    for (let key1 of Object.keys(main['contents']))
        for (let key2 of Object.keys(main['contents'][key1]))
            main['contents'][key1][key2] = main['contents'][key1][key2]
                ...
                .replaceAll('资源管理器', '文件')
                .replaceAll('运行和调试', '调试')
    await fs.promises.writeFile(path.join('locale', 'main.i18n.json'), JSON.stringify(main, null, 4))
    ...
    for await (let file of await fs.promises.opendir(path.join('.tmp', 'i18n', 'vscode-language-pack-zh-hans', 'translations', 'extensions')))
        if (file.name == 'vscode.git.i18n.json') {
            ...
            await fs.promises.writeFile(path.join('locale', 'extensions', file.name), JSON.stringify(references_view, null, 4))
        }
        else
            await fs.promises.copyFile(
                path.join('.tmp', 'i18n', 'vscode-language-pack-zh-hans', 'translations', 'extensions', file.name), 
                path.join('locale', 'extensions', file.name)
            )
} finally {
    await fs.promises.rm('.tmp', {recursive: true, force: true})
}